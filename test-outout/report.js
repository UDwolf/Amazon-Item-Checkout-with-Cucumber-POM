$(document).ready(function() {var formatter = new CucumberHTML.DOMFormatter($('.cucumber-report'));formatter.uri("C:/Selenium_Workspace/AmazonCucumberPOM/src/main/java/features/amazonSelectIteam.feature");
formatter.feature({
  "line": 1,
  "name": "Amazon bedding Iteam selection and check out testing",
  "description": "",
  "id": "amazon-bedding-iteam-selection-and-check-out-testing",
  "keyword": "Feature"
});
formatter.scenario({
  "line": 3,
  "name": "Validate Amazon Home page",
  "description": "",
  "id": "amazon-bedding-iteam-selection-and-check-out-testing;validate-amazon-home-page",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user opens Browser",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "user on Home page",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user click on Manubar-All",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user click on Shop of Department see All",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user click on Home and kitchen",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "user click on Bedding",
  "keyword": "Then "
});
formatter.step({
  "line": 11,
  "name": "user on bedding Page",
  "keyword": "Then "
});
formatter.step({
  "line": 12,
  "name": "user click on Feacture brands SafeRest and Foamily",
  "keyword": "Then "
});
formatter.step({
  "line": 13,
  "name": "title of page change according to brands",
  "keyword": "When "
});
formatter.step({
  "line": 14,
  "name": "user click on Product",
  "keyword": "Then "
});
formatter.step({
  "line": 15,
  "name": "user on a Product page",
  "keyword": "Then "
});
formatter.step({
  "line": 16,
  "name": "title on a page chnage according to Product",
  "keyword": "When "
});
formatter.step({
  "line": 17,
  "name": "user check the Ratings",
  "keyword": "Then "
});
formatter.step({
  "line": 18,
  "name": "user click on Add To Cart",
  "keyword": "Then "
});
formatter.step({
  "line": 19,
  "name": "check if iteam is alredy in cart",
  "keyword": "When "
});
formatter.step({
  "line": 20,
  "name": "click on process to checkout",
  "keyword": "Then "
});
formatter.step({
  "line": 21,
  "name": "user is on Sign IN page",
  "keyword": "Then "
});
formatter.step({
  "line": 22,
  "name": "title of the page change to Sign In",
  "keyword": "When "
});
formatter.step({
  "line": 23,
  "name": "user click on create your amazon account",
  "keyword": "Then "
});
formatter.step({
  "line": 24,
  "name": "user Enter all the details and click on create account",
  "keyword": "Then "
});
formatter.match({
  "location": "AmazonStepDefinations.user_opens_Browser()"
});
formatter.result({
  "duration": 9392282700,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_on_Home_page()"
});
formatter.result({
  "duration": 7084200,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_Manubar_All()"
});
formatter.result({
  "duration": 1707622300,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_Shop_of_Department_see_All()"
});
formatter.result({
  "duration": 3393610000,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_Home_and_kitchen()"
});
formatter.result({
  "duration": 1255612800,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_Bedding()"
});
formatter.result({
  "duration": 2580997600,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_on_bedding_Page()"
});
formatter.result({
  "duration": 1187883100,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_Feacture_brands_SafeRest_and_Foamily()"
});
formatter.result({
  "duration": 18300,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.title_of_page_change_according_to_brands()"
});
formatter.result({
  "duration": 13200,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_Product()"
});
formatter.result({
  "duration": 12300,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_on_a_Product_page()"
});
formatter.result({
  "duration": 11900,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.title_on_a_page_chnage_according_to_Product()"
});
formatter.result({
  "duration": 12300,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_check_the_Ratings()"
});
formatter.result({
  "duration": 12500,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_Add_To_Cart()"
});
formatter.result({
  "duration": 16900,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.check_if_iteam_is_alredy_in_cart()"
});
formatter.result({
  "duration": 15400,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.click_on_process_to_checkout()"
});
formatter.result({
  "duration": 20700,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_is_on_Sign_IN_page()"
});
formatter.result({
  "duration": 11800,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.title_of_the_page_change_to_Sign_In()"
});
formatter.result({
  "duration": 12400,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_click_on_create_your_amazon_account()"
});
formatter.result({
  "duration": 21900,
  "status": "passed"
});
formatter.match({
  "location": "AmazonStepDefinations.user_Enter_all_the_details_and_click_on_create_account()"
});
formatter.result({
  "duration": 13700,
  "status": "passed"
});
});